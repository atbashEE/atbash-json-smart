...
abstract class JSONParserBase {
     ...
     */
    protected <T> T parse(JSONEncoder<T> JSONEncoder) {
        this.pos = -1;
        T result;
        try {
            read();
    ...
    protected <T> T readArray(JSONEncoder<T> jsonEncoder) throws IOException {
     ...
     * generated by a ContainerFactory
     */
    protected <T> T readFirst(JSONEncoder<T> jsonEncoder) throws IOException {
        for (; ; ) {
            switch (c) {
                // skip spaces
                case ' ':
     ...
     * generated by a ContainerFactory
     */
    protected Object readMain(JSONEncoder<?> JSONEncoder, boolean stop[]) throws IOException {
        for (; ; ) {
            switch (c) {
                // skip spaces
                case ' ':
    ...
    abstract protected Object readNumber(boolean[] stop) throws IOException;

    protected <T> T readObject(JSONEncoder<T> jsonEncoder) throws IOException {
        //
        if (c != '{') {
            throw new RuntimeException("Internal Error");
        }
        Object current = jsonEncoder.createObject();
        boolean needData = false;
    ...
    protected void readString2() throws IOException {
        /* assert (c == '\"' || c == '\'') */
        char sep = c;
        for (; ; ) {
            read();
    ...
    protected char readUnicode(int totalChars) throws IOException {
        int value = 0;
