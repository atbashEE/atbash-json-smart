...
abstract class JSONParserBase {
     ...
     */
    protected <T> T parse(Mapper<T> mapper) {
        this.pos = -1;
        T result;
        try {
            read();
    ...
    protected <T> T readArray(Mapper<T> mapper) throws IOException {
     ...
     * generated by a ContainerFactory
     */
    protected <T> T readFirst(Mapper<T> mapper) throws IOException {
        for (; ; ) {
            switch (c) {
                // skip spaces
                case ' ':
     ...
     * generated by a ContainerFactory
     */
    protected Object readMain(Mapper<?> mapper, boolean stop[]) throws IOException {
        for (; ; ) {
            switch (c) {
                // skip spaces
                case ' ':
    ...
    protected <T> T readObject(Mapper<T> mapper) throws IOException {
        //
        if (c != '{') {
            throw new RuntimeException("Internal Error");
        }
